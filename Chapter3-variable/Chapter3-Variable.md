...
問題來了，如果我們已經有了 `var` 可以用來宣告變數，那為何還要大費周章的再發明一個 `let` 呢？

問題在於通過 `var` 保留詞宣告的變數會被定義為「全局變數(Global Variable)」，這種變數有一個問題點，就是在變數宣告的檔案之外的其他檔案也能讀取到這個變數：
...
如上述範例所示，當我們在 `main.js` 檔案中透過 `var` 宣告 `num` 後，我們是可以在其他檔案中例如 `other.js` 對他進行更改的。這在開發中是***極度危險*** 的行為。

在一個大型專案中，可能存在上百個檔案，每個檔案中可能都存在上百甚至上千行程式碼，後人在維護程式碼時很可能不清楚當時這個變數被宣告的原因，進而做出非預期的改動。而當程式因此出現問題時會導致追蹤與修復的難度變得非常大。因此才有了之後 `let` 的出現。
 
其實這裡的關鍵問題在於變數的「生命週期（scope）」，不過在此處對生命週期進行詳細解釋可能會讓初學者很難理解，因此關於生命週期的問題會在之後單開一小節進行解釋。在這裡只需要先知道，當我們需要宣告變數時，請優先使用 `let`。
